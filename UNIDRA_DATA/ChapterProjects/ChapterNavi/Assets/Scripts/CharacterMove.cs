...
public class CharacterMove : MonoBehaviour {
	...
	void Update () {
		...
		if (characterController.isGrounded) {
			...
			//　목적지에 가까이 왔으면 도착.
			arrived = distance < StoppingDistance;

			// 도착하면 목적지를 현재 위치로 한다.
			if (arrived)
				destinationXZ = transform.position;
			
			Vector3 velocitySave = velocity;
			velocity.y = 0; // 수평 방면만의 속도를 구하고 싶으므로 Y 방향은 0으로 해둔다.
			
			// 부드럽게 보간 처리.
			Vector3.SmoothDamp(transform.position,destinationXZ,ref velocity, 0.3f,walkSpeed);
			...
			if (!forceRotate) {
				...
				if (velocity.magnitude > 0.1f && !arrived) { 
					...
					Quaternion characterTargetRotation = Quaternion.LookRotation(velocity.normalized);
			...
			velocity.y = velocitySave.y; // Y 방향 속도를 원래로 되돌린다.
		}
